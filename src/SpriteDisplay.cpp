...
Sprite * SpriteDisplay::addSprite (const uint8_t * spriteData) {
  Sprite * sprite;
  for (uint8_t k = 0; k < MAX_SPRITES; k++) {
    sprite = &sprites[k];
    if (!sprite->active) {
...
bool SpriteDisplay::rectsOverlap (int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2) {
...
void SpriteDisplay::setDirtyRect (uint8_t x, uint8_t y, uint8_t w, uint8_t h) {
  ...
  for (uint8_t row = y / 8; row < endRow; row++) {
    for (uint8_t col = x / 4; col < endCol; col++) {
...
void SpriteDisplay::removeAllSprites () {
  Sprite * sprite;
  for (uint8_t k = 0; k < MAX_SPRITES; k++) {
    sprite = &sprites[k];
    sprite->active = false;
...
void SpriteDisplay::wipe () {
  ssd1306_fillscreen(0x00);
}

void SpriteDisplay::writeText(uint8_t x, uint8_t y, const char * str) {
	uint8_t chr = 0;
  uint8_t byte = 0;
  uint8_t index = 0;
  uint8_t fontIndex = 0;
